...
class mod_attendance_structure {
    ...
    public function url_checkin($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/attendance/checkin.php', $params);
    }
...
//        }
    ...
    public function get_session_log($sessionid) : array {
        global $DB;
        return $DB->get_records('attendance_log', array('sessionid' => $sessionid), '', 'studentid,statusid,id,remarks,timein,timeout,isonlinecheckin');
...
//              ORDER BY ats.sessdate ASC";
    ...
    public function get_user_filtered_sessions_log_extended($userid) : array {
        ...
        if ($this->get_group_mode()) {
            ...
            $sql = "SELECT $id, ats.id, ats.groupid, ats.sessdate, ats.duration, ats.description,
                           ...
                           ats.preventsharedip, ats.preventsharediptime, ats.rotateqrcode,al.timein,al.timeout, ats.onlinetime, ats.onlineduration
                      FROM {attendance_sessions} ats
            ...
            $sql = "SELECT $id, ats.id, ats.groupid, ats.sessdate, ats.duration, ats.description, ats.statusset,
                           al.statusid, al.remarks,ats.autoassignstatus,al.timein,al.timeout, ats.onlinetime, ats.onlineduration
                      FROM {attendance_sessions} ats
                RIGHT JOIN {attendance_log} al
                        ON ats.id = al.sessionid AND al.studentid = :uid
                     WHERE $where
    ...
    public function get_lowgrade_threshold() {
        if (!isset($this->lowgradethreshold)) {
            ...
            if ($this->grade > 0) {
                $gradeitem = grade_item::fetch(array(' id' => $this->course->id, 'itemtype' => 'mod',
