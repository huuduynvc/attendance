...
$PAGE->requires->js(new moodle_url('https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.js'));
...
if (($formdata = data_submitted()) && $edit == -1) {
        ...
        $teachers = $a->get_teachers_by_course_id((int)$course->id);
        $url_t = new moodle_url('/mod/attendance/take.php?id='.$id.'&sessionid='.$fb->sessionid.'&grouptype=0');
        foreach ($teachers as $teacher){
            send_notification($USER,$teacher,$cm,$course,'attendance',$fb->description,$url_t);
        }
//    $admin = get_admins();
//    foreach ($admin as $as){
//        send_notification($USER,$as,$coursemodule,$course,'attendance',$fb->description,$url_t);
//    }
